...
namespace App\Filament\Resources\CustomerNetworks\Tables;
    ...
    public static function configure(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('upline.ref_code')
                    ->label('Kode Upline')
                    ->placeholder('-')
                    ->searchable()
                    ->toggleable(),
            ...
            ->filters([
                ...
                SelectFilter::make('upline_id')
                    ->label('Upline')
                    ->relationship('upline', 'name')
                    ->searchable()
                    ->preload()
                    ->placeholder('Semua upline'),
                ...
                TernaryFilter::make('has_upline')
                    ->label('Ketersediaan Upline')
                    ->placeholder('Semua')
                    ->trueLabel('Ada upline')
                    ->falseLabel('Root (tanpa upline)')
                    ->queries(
                        true: fn (Builder $query): Builder => $query->whereNotNull('upline_id'),
                        false: fn (Builder $query): Builder => $query->whereNull('upline_id'),
            ...
            ->filtersFormSchema(fn (array $filters): array => [
                ...
                $filters['upline_id']->columnSpan(3),
                $filters['position']->columnSpan(2),
                $filters['status']->columnSpan(2),
                $filters['has_upline']->columnSpan(2),
