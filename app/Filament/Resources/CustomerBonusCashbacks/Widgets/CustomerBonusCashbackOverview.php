...
namespace App\Filament\Resources\CustomerBonusCashbacks\Widgets;
...
class CustomerBonusCashbackOverview extends StatsOverviewWidget
{
    protected ?string $heading = 'Insight Bonus Cashback';

    protected ?string $description = 'Ringkasan distribusi dan status bonus cashback member.';
    ...
    {
        $data = Cache::remember(
            CustomerBonusCashback::CACHE_KEY_OVERVIEW,
            now()->addMinutes(10),
            fn (): array => $this->computeStats(),
        );

        return [
            Stat::make('Total Bonus Cashback', $this->formatNumber($data['totalRecords']))
            ...
            Stat::make('Rata-rata Cashback', $this->formatCurrencyIdr($data['averageAmount']))
                ->description('Per record dari total ' . $this->formatNumber($data['totalRecords']) . ' cashback')
    ...
    /**
     * @return array{totalRecords: int, totalAmount: float, pendingCount: int, pendingAmount: float, releasedCount: int, releasedAmount: float, averageAmount: float, chart: array<int, int>}
     */
    protected function computeStats(): array
    {
        $bonusQuery = CustomerBonusCashback::query();
    ...
    protected function getLast7DaysTrend(): array
    {
        ...
        $dailyCounts = CustomerBonusCashback::query()
