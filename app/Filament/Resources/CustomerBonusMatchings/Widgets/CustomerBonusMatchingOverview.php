...
namespace App\Filament\Resources\CustomerBonusMatchings\Widgets;
...
class CustomerBonusMatchingOverview extends StatsOverviewWidget
{
    protected ?string $heading = 'Insight Bonus Matching';

    protected ?string $description = 'Ringkasan distribusi dan status bonus matching member.';
    ...
    {
        $data = Cache::remember(
            CustomerBonusMatching::CACHE_KEY_OVERVIEW,
            now()->addMinutes(10),
            fn (): array => $this->computeStats(),
        );

        return [
            Stat::make('Total Bonus Matching', $this->formatNumber($data['totalRecords']))
    ...
    /**
     * @return array{totalRecords: int, totalAmount: float, pendingCount: int, pendingAmount: float, releasedCount: int, releasedAmount: float, averageAmount: float, averageLevel: float, chart: array<int, int>}
     */
    protected function computeStats(): array
    {
        $bonusQuery = CustomerBonusMatching::query();
        ...
        return [
            ...
            'averageLevel' => (float) (clone $bonusQuery)->avg('level'),
    ...
    protected function getLast7DaysTrend(): array
    {
        ...
        $dailyCounts = CustomerBonusMatching::query()
