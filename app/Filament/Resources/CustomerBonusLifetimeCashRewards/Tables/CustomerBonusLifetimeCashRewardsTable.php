...
namespace App\Filament\Resources\CustomerBonusLifetimeCashRewards\Tables;
...
class CustomerBonusLifetimeCashRewardsTable
{
    public static function configure(Table $table): Table
    {
        return $table
            ->recordTitleAttribute('CustomerBonusLifetimeCashReward')
            ...
            ->filters([
                ...
                Filter::make('reward_range')
                    ->label('Rentang Nilai Target')
                    ->schema([
                        TextInput::make('min')
                            ->label('Minimum (Rp)')
                            ->numeric()
                    ...
                    ->query(
                        fn (Builder $query, array $data): Builder => $query
                            ->when(filled($data['min'] ?? null), fn (Builder $builder): Builder => $builder->where('reward', '>=', $data['min']))
                            ->when(filled($data['max'] ?? null), fn (Builder $builder): Builder => $builder->where('reward', '<=', $data['max']))
                    )
                    ->indicateUsing(function (array $data): array {
                        $indicators = [];

                        if (filled($data['min'] ?? null)) {
                            $indicators[] = Indicator::make('Target dari Rp' . number_format((float) $data['min'], 0, ',', '.'))->removeField('min');
                        }

                        if (filled($data['max'] ?? null)) {
                            $indicators[] = Indicator::make('Target sampai Rp' . number_format((float) $data['max'], 0, ',', '.'))->removeField('max');
                        }
                ...
                Filter::make('amount_range')
                    ->label('Rentang Nominal Diterima')
                    ->schema([
                        TextInput::make('min')
                            ->label('Minimum (Rp)')
                            ->numeric()
                    ...
                    ->indicateUsing(function (array $data): array {
                        ...
                        if (filled($data['min'] ?? null)) {
                            $indicators[] = Indicator::make('Diterima dari Rp' . number_format((float) $data['min'], 0, ',', '.'))->removeField('min');
                        }

                        if (filled($data['max'] ?? null)) {
                            $indicators[] = Indicator::make('Diterima sampai Rp' . number_format((float) $data['max'], 0, ',', '.'))->removeField('max');
                        }
            ...
            ->filtersFormSchema(fn (array $filters): array => [
                ...
                $filters['reward_range']->columnSpan(6),
                $filters['amount_range']->columnSpan(6),
