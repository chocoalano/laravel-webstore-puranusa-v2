...
namespace App\Filament\Resources\CustomerBonusRetails\Widgets;
...
class CustomerBonusRetailOverview extends StatsOverviewWidget
{
    protected ?string $heading = 'Insight Bonus Retail';

    protected ?string $description = 'Ringkasan distribusi dan status bonus retail member.';
    ...
    {
        $data = Cache::remember(
            CustomerBonusRetail::CACHE_KEY_OVERVIEW,
            now()->addMinutes(10),
            fn (): array => $this->computeStats(),
        );

        return [
            Stat::make('Total Bonus Retail', $this->formatNumber($data['totalRecords']))
            ...
            Stat::make('Rata-rata Bonus', $this->formatCurrencyIdr($data['averageAmount']))
                ->description('Member unik: ' . $this->formatNumber($data['uniqueMembers']))
    ...
    {
        $bonusQuery = CustomerBonusRetail::query();
    ...
    protected function getLast7DaysTrend(): array
    {
        ...
        $dailyCounts = CustomerBonusRetail::query()
