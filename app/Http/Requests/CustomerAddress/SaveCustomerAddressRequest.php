...
class SaveCustomerAddressRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'province_id'    => [
                ...
                'integer',
                'min:1',
                Rule::exists('shipping_targets', 'province_id'),
            ],
            ...
            'city_id'        => [
                ...
                'integer',
                'min:1',
                Rule::exists('shipping_targets', 'city_id')
                    ->where(fn ($query) => $query->where('province_id', (int) $this->integer('province_id'))),
            ],
            'district'       => [
                'required',
                'string',
                'max:255',
                Rule::exists('shipping_targets', 'district')
                    ->where(
                        fn ($query) => $query
                            ->where('province_id', (int) $this->integer('province_id'))
                            ->where('city_id', (int) $this->integer('city_id'))
                    ),
            ],
    ...
    public function messages(): array
    {
        return [
            ...
            'province_id.exists'      => 'Provinsi tidak ditemukan pada target pengiriman.',
            ...
            'city_id.exists'          => 'Kota/kabupaten tidak ditemukan untuk provinsi terpilih.',
            'district.required'       => 'Kecamatan wajib dipilih.',
            'district.exists'         => 'Kecamatan tidak ditemukan untuk kota/provinsi terpilih.',
        ];
    }
